...
public class DI_AI extends AIWithComputationBudget {
    ...
    public DI_AI(UnitTypeTable utt) {
        super(-1, -1);
        _utt = utt;
    ...
    boolean produce(Unit u, int dir, UnitType bType) {
        ...
        else if (bType == _utt.getUnitType("Ranged"))
            _futureHeavies += 1;
    ...
    int combatScore(Unit u, Unit e) {
        ...
        if (_pgs.getWidth() >= 16 && (u.getType() == _utt.getUnitType("Ranged") || u.getType() == _utt.getUnitType("Ranged"))
    ...
    int buildBarrackWorkerScore(Pos dst, Unit w) {
        ...
        int heavyTLen = _utt.getUnitType("Ranged").produceTime  /  _utt.getUnitType("Worker").moveTime;
    ...
    void init() {
        ...
        for (Unit u : _all) { //todo big change that was ally by mistake
            ...
            if (!isEnemyUnit(u) && (ut != null))
             ...
             if (!isEnemyUnit(u) && ut == _utt.getUnitType("Ranged")) {
                 _futureHeavies += 1;
             }
